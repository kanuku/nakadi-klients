...
class EventIntegrationHelper(generator: EventGenerator, client: Client)
    extends WordSpec
    with Matchers {
  ...
  def getEventType(eventTypeName: String = eventType.name): Option[EventType] =
    executeCall(client.getEventType(eventTypeName)) match {
  ...
  def getEnrichmentStrategies(): Seq[EventEnrichmentStrategy.Value] =
    extractFromRightOptional(() => client.getEnrichmentStrategies())
  def getPartitionStrategies(): Seq[PartitionStrategy.Value] =
    extractFromRightOptional(() => client.getPartitioningStrategies())

  //Private methods
  private def wasItsuccessfull(in: Option[ClientError], msg: String): Boolean =
    in match {
