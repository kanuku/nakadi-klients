...
trait JavaClientHandler {
  def deserialize[T](response: HttpResponse,
                     des: Deserializer[T]): Future[Optional[T]]
  def get[T](endpoint: String,
             des: Deserializer[T]): java.util.concurrent.Future[Optional[T]]
  def get[T](endpoint: String,
             headers: Seq[HttpHeader],
             des: Deserializer[T]): java.util.concurrent.Future[Optional[T]]
  def post[T](endpoint: String, model: T)(
      implicit serializer: Serializer[T]): java.util.concurrent.Future[Void]
  def subscribe[T <: JEvent](eventTypeName: String,
                             endpoint: String,
                             parameters: JStreamParameters,
                             listener: JListener[T])(
      implicit des: Deserializer[JEventStreamBatch[T]]): Optional[ClientError]
  def unsubscribe[T <: JEvent](eventTypeName: String,
...
class JavaClientHandlerImpl(val connection: Connection,
                            subscriber: SubscriptionHandler)
    extends JavaClientHandler {
  ...
  def get[T](
    ...
    FutureConversions.fromFuture2Future(
        connection.get(endpoint).flatMap(deserialize(_, des)))
  }
  def get[T](
      endpoint: String,
      headers: Seq[HttpHeader],
  ...
  def post[T](endpoint: String, model: T)(implicit serializer: Serializer[T])
    ...
    val result = connection
      .executeCall(
          withHttpRequestAndPayload(endpoint,
                                    serialize(model),
                                    HttpMethods.POST,
                                    connection.tokenProvider))
      .flatMap(response(_))
  ...
  private def serialize[T](model: T)(
      implicit serializer: Serializer[T]): String =
    ...
    response match {
      ...
      case HttpResponse(status, headers, entity, protocol)
        ...
        Unmarshal(entity).to[String].map { x =>
          val msg = "http-stats(%s) - %s - problem: %s "
            .format(status.intValue(), x, status.defaultMessage())
          logger.warn(msg)
  ...
  def subscribe[T <: JEvent](eventTypeName: String,
                             ...
                             parameters: JStreamParameters,
                             listener: JListener[T])(
      implicit des: Deserializer[JEventStreamBatch[T]]) = {
    import ModelConverter._
    val params: Option[ScalaStreamParameters] = toScalaStreamParameters(
        parameters)
    val eventHandler: EventHandler = new JavaEventHandlerImpl(des, listener)
    val finalUrl = withUrl(endpoint, params)
    val res = subscriber
      .subscribe(eventTypeName, finalUrl, getCursor(params), eventHandler)
  ...
  private def getCursor(
      params: Option[ScalaStreamParameters]): Option[ScalaCursor] =
    params match {
      case Some(
