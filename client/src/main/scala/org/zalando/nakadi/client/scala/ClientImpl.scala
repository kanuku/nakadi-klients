...
class ClientImpl(connection: Connection,
  ...
  def getMetrics(): Future[Either[ClientError, Option[Metrics]]] = {
    logFutureEither(
        connection
          .get(URI_METRICS)
          .flatMap(mapToEither(_)(deserializer(metricsTR))))
  }
  ...
  def getEventTypes(): Future[Either[ClientError, Option[Seq[EventType]]]] = {
    logFutureEither(
        connection
          .get(URI_EVENT_TYPES)
          .flatMap(mapToEither(_)(deserializer(listOfEventTypeTR))))
  }

  def createEventType(eventType: EventType): Future[Option[ClientError]] = {
    logFutureOption(
        connection.post(URI_EVENT_TYPES, eventType).flatMap(mapToOption(_)))
  }

  def getEventType(
      name: String): Future[Either[ClientError, Option[EventType]]] = {
    logFutureEither(
        connection
          .get(URI_EVENT_TYPE_BY_NAME.format(name))
          .flatMap(in => mapToEither(in)(deserializer(eventTypeTR))))
  }

  def updateEventType(name: String,
  ...
  def deleteEventType(name: String): Future[Option[ClientError]] = {
    logFutureOption(
        connection
          .delete(URI_EVENT_TYPE_BY_NAME.format(name))
          .flatMap(in => mapToOption(in)))
  }
  ...
  def publishEvents[T <: Event](
    ...
    logFutureOption(
        connection
          .post(URI_EVENTS_OF_EVENT_TYPE.format(eventTypeName), events)
          .flatMap(in => mapToOption(in)))
  }
  def publishEvents[T <: Event](
      eventTypeName: String,
  ...
  def getPartitions(
      name: String): Future[Either[ClientError, Option[Seq[Partition]]]] = {
    logFutureEither(
        connection
  ...
  def getEnrichmentStrategies(): Future[
      Either[ClientError, Option[Seq[EventEnrichmentStrategy.Value]]]] = {
    logFutureEither(
        connection
          .get(URI_ENRICHMENT_STRATEGIES)
          .flatMap(
              mapToEither(_)(deserializer(listOfEventEnrichmentStrategyTR))))
  }
  ...
  def getPartitioningStrategies()
    ...
    logFutureEither(
        connection
          .get(URI_PARTITIONING_STRATEGIES)
          .flatMap(mapToEither(_)(deserializer(listOfPartitionStrategyTR))))
  ...
  def subscribe[T <: Event](eventTypeName: String,
    ...
    (eventTypeName, params, listener) match {
      ...
      case (eventType, StreamParameters(cursor, _, _, _, _, _, _), listener)
          if Option(eventType).isDefined =>
        val url = URI_EVENTS_OF_EVENT_TYPE.format(eventType)
        logger.debug(
            "Subscribing listener {} - cursor {} - parameters {} - eventType {} - url {}",
            listener.id,
            cursor,
            params,
            eventType,
        ...
        val eventHandler: EventHandler =
          new ScalaEventHandlerImpl(des, listener)
  ...
  private def logFutureEither[A, T](future: Future[Either[ClientError, T]])
  ...
  private def mapToEither[T](response: HttpResponse)(
    ...
    : Future[Either[ClientError, Option[T]]] = {
    ...
    response match {
      case HttpResponse(status, headers, entity, protocol)
        ...
        try {
          ...
          case e: Throwable =>
            val msg =
              "Failed to deserialise the content with error: %s".format(
                  e.getMessage)
            logger.error(msg)
      ...
      case HttpResponse(StatusCodes.NotFound, headers, entity, protocol) =>
        logger.info(
            s"Received: httpStatus - Not found ${StatusCodes.NotFound}")
        Future.successful(Right(None))
      case HttpResponse(status, headers, entity, protocol)
          if (status.isRedirection()) =>
        val msg = "Not implemented: http-status (" + status
            .intValue() + "}) and reason:" + status.reason()
        logger.info(msg)
        Future.successful(Left(ClientError(msg, Some(status.intValue()))))
      case HttpResponse(status, headers, entity, protocol) =>
        Unmarshal(entity).to[String].map { body =>
          val msg = "Service return http-status: %s (%s) Message: %s"
            .format(status.intValue(), status.reason(), body)
          logger.warn(msg)
  ...
  private[client] def mapToOption[T](
      response: HttpResponse): Future[Option[ClientError]] = {
    response.status match {
      case status if (status.isSuccess()) =>
        logger.info(s"Success. http-status: ${status.intValue()}")
        response.entity.toStrict(10.second).map { body =>
          logger.debug(
              "Success - http-status: %s, body:[%s]".format(
                  status.intValue().toString(),
                  body.data.decodeString(charSet)))
        }
        Future.successful(None)
      case status if (status.isRedirection()) =>
        val msg =
          s"Redirection - http-status: ${status.intValue()}, reason[${status.reason()}]"
        logger.info(msg)
        response.entity.toStrict(10.second).map { body =>
          logger.debug(
              s"Redirection - http-status: ${status.intValue().toString()}, reason[${status
      ...
      case status if (status.isFailure()) =>
        ...
        response.entity.toStrict(10.second).map { body =>
          val msg =
            s"Failure - http-status: ${status.intValue()}, reason[${status
              .reason()}], body:[${body.data.decodeString(charSet)}]"
          logger.warn(msg)
