 ...
 * @param partitionKeyFields Indicator of the field used for guaranteeing the ordering of Events of this type (used by the PartitionResolutionStrategy). If set MUST be a valid required field as defined in the schema.
...
case class EventType(
  ...
  @JsonScalaEnumeration(classOf[EventValidationStrategyType]) validationStrategies: Seq[EventValidationStrategy.Value],
...
case object EventValidationStrategy extends Enumeration {
  type EventValidationStrategy = Value
  val SCHEMA_VALIDATION = Value("schema-validation")
}
